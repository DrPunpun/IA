...
class BayesNet:
    ...
    def genConjunctions(self, var_list):
        if len(var_list) == 1:
            return [[(var_list[0], True)], [(var_list[0], False)]]
        l = []
        for r in self.genConjunctions(var_list[1:]):
            l.append(r + [(var_list[0], True)])
            l.append(r + [(var_list[0], False)])
        return l

    def individualProb(self, var, val):
        all_vars = self.dependencies.keys()
        todas_conj = self.genConjunctions([v for v in all_vars if v!= var])
